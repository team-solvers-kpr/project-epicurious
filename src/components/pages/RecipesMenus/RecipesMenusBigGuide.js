      ...
      <div className="grid lg:grid-cols-2 grid-cols-1 divide-x py-8">
        <div className="lg:pl-[16rem] pl-10 pr-4">
          ...
          <p className="text-sm mt-8 lg:text-start text-center">
            For these recipes, you'll make a double batch of a base sauce. Use
            half for one dish, and then turn the remainder into a radically
            different meal the following night.
          </p>
        ...
        <div className="lg:pl-10 pl-4 lg:pr-64 pr-4 lg:pt-0 pt-12">
          {bigGuideBottom.map((data) => (
            <TheBigGuideBottom key={data.id} data={data}></TheBigGuideBottom>
          ))}
        </div>
      </div>
    </div>
